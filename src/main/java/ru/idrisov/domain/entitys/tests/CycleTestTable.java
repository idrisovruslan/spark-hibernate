...
@Cycle(cycleName = "NESTED", nestedCycleName = "NESTED_NESTED", sourceTableField = @SourceTableField(sourceTable = HubSyntAcctTable.class, sourceFieldName = "num",
...
@Cycle(cycleName = "NESTED_NESTED", sourceTableField = @SourceTableField(sourceTable = HubSyntAcctTable.class, sourceFieldName = "num",
        //TODO Подумать над более интуитивно понятным решением
        conditions = {@WhereCondition(leftFieldName = "syntacct_type", stringRightValue = "LVL3"),
                @WhereCondition(leftValueFunction = "substring(%s, 1, 5)", cycleRightValue = "NESTED")}))

@Distinct
public class CycleTestTable implements TableSpark {
